...
#include <iomanip>
using namespace std;
const string ClientsFileName = "C:\\Users\\benkaida salah\\Desktop\\bankclient.txt";
void ShowMainMenue();
...
sClient ReadNewClient()
{
	...
	while (ClientExistsByAccountNumber(Client.AccountNumber,
		ClientsFileName))
	{
		cout << "\nClient with [" << Client.AccountNumber << "] already exists, Enter another Account Number ? ";getline(cin >> ws, Client.AccountNumber);
	}
	cout << "Enter PinCode? ";
...
void PrintClientRecordLineForTotalBalancesScreen(sClient Client)
{
	cout << "| " << setw(15) << left << Client.AccountNumber;
...
{
	char TransPerforme;
	cout<< "Are Your You Want to Performe This Transaction (Y/N) ? \n";
	cin >> TransPerforme;
	if (TransPerforme == 'Y' or TransPerforme == 'y')
	{
		for (sClient& C : vClients)
		{
			if (C.AccountNumber == AccountNumber)
...
void ShowDepositScreen()
{
		...
		if (FindClientByAccountNumber(AccountNumber, vClients, Client))
		{
			...
			cin >> DepositAmount;
			DepositBalanceByAccountNumber(AccountNumber, vClients, DepositAmount);
		}
		else
		{
			cout << "\nClient with Account Number[" << AccountNumber << "] is not found!";
...
void ShowTotalBalancesScreen()
{
	...
	if (vClients.size() == 0)
		...
		for (sClient Client : vClients)
		{
			...
			PrintClientRecordLineForTotalBalancesScreen(Client);
			cout << endl;
		}
	cout <<"\n_______________________________________________________";
	cout << "_________________________________________\n" << endl;
...
void PerformTransactionsMenuOption(enTransactionsMenueOptions TransactionsOption)
{
...
void ShowTransactionsScreen()
{
	...
	cout << "\t[4] Main Menue.\n";
	cout << "===========================================\n";
	PerformTransactionsMenuOption((enTransactionsMenueOptions)ReadTransactionMenueOption());
}
int main()
{
	ShowMainMenue();
